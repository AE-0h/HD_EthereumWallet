...
const testNet = ethers.getDefaultProvider('ropsten');
    ...
    fs.writeFile(walletDirectory + filename, jsonWallet, 'utf-8', (err)=>{
      if (err){
        drawView(res, 'create', {
          ...
          jsonWallet: undefined,
          filename: undefined,
          error: 'Problem writting to disk' + err.message,
        });
    }else{
      drawView(res, 'create', {
    ...
    ethers.Wallet.fromEncryptedJson(jsonWallet, password)
      ...
      .catch((err) => {
        drawView(res, 'load', {
          ...
          privateKey: undefined,
          mnemonic: undefined,
          error: 'Wrong password' + err.message,
        });
      });
  });
});
...
app.post('/balance', (req, res) => {
        ...
        ethers.Wallet.fromEncryptedJson(jsonWallet, password)
          .then(async (wallet) => {
            ...
            for (let i = 0; i < 5; i++) {
              ...
              let walletInstance = new ethers.Wallet(hdNode.privateKey, testNet);
      ...
      app.post('/send', (req, res) => {
        ...
        try {
          ...
          wallet = new ethers.Wallet(privateKey, testNet);
